...
while read -r FAMID CRAMSLIST; do
  ...
  aws batch submit-job \
      ...
      --job-definition ssc-denovos-advntr:1 \
      --container-overrides 'command=["run_advntr.sh",'"${CRAMSLIST}"','"${FAMID}"','"${ENC_SSC_ACCESS_KEY}"','"${ENC_SSC_SECRET_ACCESS_KEY}"', '"${SSC_PARAMS}"'],environment=[{name="BATCH_FILE_TYPE",value="script"},{name="BATCH_FILE_S3_URL",value="s3://ssc-gangstr-denovos/scripts/run_advntr.sh"}]'
